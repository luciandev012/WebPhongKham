...
const TOAST_DEFAULT_POSITION = 'bottom-left';
...
function renderNotify() {
	...
	if (!$('#toast-container').length) {
		const position = ['top-right', 'top-left', 'top-center', 'bottom-right', 'bottom-left', 'bottom-center'].includes(NotifyOption.position) ? NotifyOption.position : TOAST_DEFAULT_POSITION;
...
const Toast = {
	show: (content, title, delay, position) => {
		...
		NotifyOption.title = !title || title == '' ? "ThÃ´ng bÃ¡o" : title;
